...
\begin{define}(Operator für $ \vdKDR $)
	\begin{align*}
		&F_h^n: V_h \to V_h \text{ gegeben durch }\\ & \qquad (F_h^n( \cdot ), \phi_h) = \bigg( \frac{1}{\Delta t} M_h \, \cdot + A_h \, \cdot, \phi_h  \bigg)_\Omega + \bigg( R_h^n( \cdot ), \phi_h \bigg)_\Omega - \bigg( \frac{1}{\Delta} M_h c_h^{\textcolor{red}{n-1}}, \phi_h \bigg)_\Omega \\
