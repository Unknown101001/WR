...
\end{itemize}
Die Diskreditierung erfolgt wieder zunächst im Raum und dann in der Zeit:
...
als ODE in $ \R^N $.\\
Nun Diskreditieren wir mit dem impl. Euler-Verfahren auch in der Zeit und erhalten für $ t_n \coloneqq n \, \Delta t  $ das voll-diskrete Problem
\begin{gather*}
\vdKDR \begin{cases}
...
\begin{itemize}
	...
	\item \defi{Petrov-Gaerkin}: Lösungsraum und Testraum unterschiedlich\\ Lösungsfunktion $ c_h \in V_h $ und Testfunktion $ \phi_h + \delta_K q \cdot \nabla \phi_h $ auf $ K \in \mathcal{K} $ und mit $ \delta_K > 0 $.
\end{itemize}
...
\begin{define}(\defi{Pekle-Zahl})
	\[ \Pe_K \coloneqq \frac{h \, \norm{q}_{\infty, K}}{\norm{\kappa}_{\infty, K}} \]
\end{define}

\begin{define}(\defi{Streamline-Deffusion-Norm})
