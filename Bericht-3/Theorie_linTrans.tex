...
\begin{align*}
	...
	& \mkern-12mu \stackrel{\substack{\text{Kanten} \\ \text{Nullmengen}}}{\iff} &&\sum_{K \in \mathcal{K}} \left(\partial_t \rho, \phi  \right)_K  = - \left(\Psi(\rho), \nabla \phi  \right)_\Omega - \sum_{K \in \mathcal{K}} \sum_{F \in \mathcal{F}_K} \left(\Psi(\rho) \cdot n^K, \phi \right)_{F} \\
...
Zunächst betrachten wir den Finite-Volumen-Ansatzraum $ Q_h = \Pi_{K \in \mathcal{K}} \mathbb{P}_0 $. 
...
\begin{Lemma}[diskrete Nicht-Negativität] \label{disktrete Nicht-Negativität}
		Sei $ q = q_h \in W_h, \dive(q) = 0 $ und $ \rho_o \in Q_h $.%Es gelten die gleichen Voraussetzungen wie in Lemma \ref{diskrete Massenbilanz}.
	
	Dann gilt für die semidiskrete Approximation $ \rho_h $ die \defi{diskrete Nicht-Negativität}:
...
%	\end{itemize}
	...
	\begin{tabular}{c || c | c | c}
		&\enquote{upwind flux} $ \Psi* $ & \enquote{central flusx} $ \Psi^c $ & Bedeutung \\ \hline 
...
%\[ = - \int_{0}^{t} \left( \int_{\Gamma_{\text{out}}} \rho_h \, q \cdot n \diff a  + \int_{\Gamma_{\text{in}}} \rho_{\text{in}} \, q \cdot n \diff a \right)  \diff \tilde{t}\]