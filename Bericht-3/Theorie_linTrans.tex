...
\begin{align*}
	...
	\iff & \left(\partial_t \rho, \phi  \right)_\Omega  = \left(\dive(\Psi(\rho)), \phi  \right)_\Omega - \sum_{\substack{K \in \mathcal{K} \\  \partial K \not\subseteq \Gammain}} \left(\Psi(\rho) \cdot n^K, \phi \right)_{\partial K} - \sum_{\substack{K \in \mathcal{K} \\  \partial K \subseteq \Gammain}} \left(\rhoin q \cdot n^K, \phi \right)_{\partial K} 
\end{align*}
\begin{remark}
...
Wähle Lösungs-/Ansatzraum $ Q_h $ mit Zellenbasis $ \{ \eta_K \}_{K\in \mathcal{K}} $ und \defi{numerical flux} $ \Psi^* $ 



Mit folgender Berechnung erhalten wir $ \partial_t \rho $ der semidiskrete Lösung $ \rho_h: [0,T] \to Q_h $ 
