...
Zunächst betrachten wir den Finite-Volumen-Ansatzraum $ Q_h = \Pi_{K \in \mathcal{K}} \mathbb{P}_0 $ und als numerischen Fluss den \defi{upwind flux} $ \Psi^* $. Dieser ist gegeben für alle $ K \in \mathcal{K} $ und $ F \in \mathcal{F}_K $ durch:
\begin{gather*}
...
\begin{remark}
	...
	Gilt  $q\cdot n^K_{|F} < 0$ so ist $ \Psi^*(\rho_h) = \Psi(\rho_{h|K_F}) $.
\end{remark}
\bigskip
...
\begin{align*}
	...
	&= \bigg( - \sum_{\substack{F \in \mathcal{F}_{K_i}, F \not\subseteq \Gammain\\ F \text{ mit }q \cdot n^K_{|F} > 0} } \left(\Psi^*(\rho_h) \cdot n^{K_i}, \mu_i \right)_{F} - \sum_{\substack{F \in \mathcal{F}_{K_i}, F \not\subseteq \Gammain\\ F \text{ mit }q \cdot n^K_{|F} < 0} } \left(\Psi^*(\rho_h) \cdot n^{K_i}, \mu_i \right)_{F}\\
...
\begin{Lemma}[diskrete Massenbilanz] \label{diskrete Massenbilanz}
	Sei $ q = q_h \in W_h, \dive(q) = 0 $ und $ \rho_o \in Q_h $.
	
	Dann gilt für die semidiskrete Approximation $ \rho_h $ die \defi{diskrete Massenbilanz}:
...
\begin{Lemma}[diskrete Positivität] \label{disktrete Positivität}
		Es gelten die gleichen Voraussetzungen wie in Lemma \ref{diskrete Massenbilanz}.
	
	Dann gilt für die semidiskrete Approximation $ \rho_h $ die \defi{diskrete Positivität}:
	\[ \rho_0 \geq 0 \text{ und } \rhoin \geq 0 \implies \rho_h \geq 0 \]
\end{Lemma}

\begin{Lemma}[diskrete Energiebilanz] \label{diskrete Energiebilanz}\unklar{Welche Voraussetzungen???}
	
	Dann gilt für die semidiskrete Approximation $ \rho_h $ die \defi{diskrete Energiebilanz}:
