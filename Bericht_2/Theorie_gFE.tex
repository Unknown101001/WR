...
Um die Flussbilanz zu erfüllen verwenden wir \defi{gemischte Finiten Elemente}. Bei dieser Methode wird, im Gegensatz zu (linearen) Finiten Elementen, nicht nur $ u_h $ als numerische Approximation für $ u $ berechnet sonder das Paar $ (u_h,q_h) $ als numerische Approximation an $ (u,q) $. Wie genau dies funktioniert, untersuchen wir im Folgenden:
...
\begin{gather*}
 \text{Bestimme } u:\overline{\Omega} \to \R \text{ mit }\\	
	...
	\end{cases}
\end{gather*}
($ \kappa $ ist sym pos-def) und setzen den \defi{Fluss} als: 
\[ q :\equiv - \kappa \nabla u. \]
Somit können wir unser analytischen Problem umschrieben zu 
\begin{gather*}
	\text{Bestimme } u:\overline{\Omega} \to \R \text{ und } q: u:\overline{\Omega} \to \R \text{ mit } \\
		...
		\end{cases}
\end{gather*} 
der \defi{Laplace-Gleichung mit Fluss}. Nun schaue wir uns, wie zuvor bei finiten Elementen, die schwache Formulierung von (LG F) an.
\begin{align*}
...
\end{align*}
Wir wollen dann noch die Dirichlet-Randbedingung $ 	u = u_D \text{ auf } \Gamma_D $ mit in die schwache Formulierung einfließen lassen. Dazu verwenden wir den Satz von Gauß:
...
Die Neumann-Randbedingung $ (\kappa\nabla u) \cdot n = g_N \text{ auf } \Gamma_N $ werden durch die Wahl des Lösungs- bwz. Ansatzraum erfüllt.
\bigskip
...
\end{gather*}
\unklar{\textbf{Idee der gemischten Finiten Elemente}:Diskretisieren wir (gFE) erfüllt unsere numerische Lösung $ q_h $ die Forderung $ \dive(q_h) = 0 \text{ auf } K \ (K \in \mathcal{K}) $ und erfüllt somit die Flussbilanz in jeder Zelle.}
...
\end{align*}
Wir nummerier Seiten und Zellen durch:
\begin{align*}
...
\begin{define}
	...
	\end{align*}
	\begin{align*}
		W_h(g) &\coloneqq \left\{ \psi_h \in W_h: \int_F \psi_h \cdot \diff a = \int_F g \diff a (F \subseteq \Gamma_N) \right\}.
	\end{align*}
	...
	\end{align*}
	und  $ \{ \mu_i \}_{m=1}^{\abs{\mathcal{K}}} $ heißt \defi{Zellenbasis} uns ist gegeben durch
	\begin{align*}
...
\begin{remark}
	...
	gilt
	\begin{align*}	
	W_h &\subseteq \bigcap_{K \in \mathcal{K}} \tilde{\mathbb{P}}_1(K,\R^2) &&\text{(Menge der zellenweise linearen Funktionen) und }\\
	Q_h &\subseteq \bigcap_{K \in \mathcal{K}} \tilde{\mathbb{P}}_0(K,\R) &&\text{(Menge der zelleiweise konstatnen Funktionen)}. 
	\end{align*}	
\end{remark}
...
\end{cases} \]
\begin{remark}
	Für alle Zellen $ K \in \mathcal{K} $ von den $ F_j $ eine anliegende Seite ist ($ \overline{K} \cap F_j \ne \emptyset $) und $ F_j $ lokal mit $ i \in \{1,2,3\} $ nummeriert ist gilt
	\[ \psi_j|_K = \psi_i^K. \]
\end{remark}

\subsection{Formulierung als LGS}
Sei, wie oben, Seiten, Seitenbasis, Zellen und Zellenbasis global nummeriert 
\begin{align*}
...
\textbf{Optimierungstheorie:} 
\begin{gather*}
	(\underline{q}, \underline{u}) \text{ Sattelpunkt von } L  \text { wie in Lemma \ref{Sattelpunkt}}\\
	\iff \underline{q} \text{ minimiert } E:W_h(-g_N) \to \R , \underline{x} \mapsto \frac{1}{2} \underline{x}^T \underline{A} \underline{x} - \underline{b}^T \underline{x} \text{ under der NB } \underline{B} \underline{q} = 0.
\end{gather*}
